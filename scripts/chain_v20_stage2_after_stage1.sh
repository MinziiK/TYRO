#!/usr/bin/env bash
# Watcher: wait until v20 STAGE 1 finishes, then auto-launch STAGE 2.
#
# "Finished" = the stage-1 training python process (matched by run-name) is no
# longer running AND a stage-1 final/last checkpoint exists. Guards against
# launching stage 2 if stage 1 died before producing any checkpoint.
set -uo pipefail
REPO="$(cd "$(dirname "$0")/.." && pwd)"
cd "$REPO"

STAGE1_PAT="src.train.*nut_fastening_v20_stage1"
LOG="runs/chain_v20_stage2.log"
echo "[chain] $(date '+%F %T') watching for v20 stage1 completion..." | tee -a "$LOG"
...
exec bash scripts/run_b_nut_train_v20_stage2.sh
