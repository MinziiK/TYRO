#!/usr/bin/env bash
# Periodic health monitor for v21 nut training pipeline.
...
# - Logs anomalies (fps drop, errors) to runs/monitor_v21.log
set -uo pipefail
REPO="$(cd "$(dirname "$0")/.." && pwd)"
cd "$REPO"
LOG="runs/monitor_v21.log"
INTERVAL=300  # 5 min
...
ensure_chain_watcher() {
  if pgrep -f "chain_v21_stage2_after_stage1" >/dev/null 2>&1; then
    return 0
  fi
  if pgrep -f "src.train.*nut_fastening_v21_stage1" >/dev/null 2>&1; then
    log "chain watcher missing — relaunching"
    setsid bash scripts/chain_v21_stage2_after_stage1.sh >> runs/chain_v21_stage2.log 2>&1 &
    disown
  fi
}
...
restart_stage1() {
  local ckpt
  ckpt="$(latest_ckpt runs/nut_fastening_v21_stage1)"
  if [[ -n "${ckpt}" ]]; then
    ...
    setsid bash -c "
      ...
      python -u -m src.train \
        ...
        >> runs/nut_fastening_v21_stage1.log 2>&1
    " &
    disown
  else
    log "RESTART stage1 fresh (no ckpt)"
    setsid bash scripts/run_b_nut_train_v21_stage1.sh >> runs/nut_fastening_v21_stage1.log 2>&1 &
    disown
  fi
}
...
while true; do
  ...
  if pgrep -f "src.train.*nut_fastening_v21_stage1" >/dev/null 2>&1; then
    ensure_chain_watcher
    steps="$(grep -a 'total_timesteps' runs/nut_fastening_v21_stage1.log 2>/dev/null | tail -1 \
             | grep -oE '[0-9]+' | tail -1)"
    fps="$(grep -a '|    fps' runs/nut_fastening_v21_stage1.log 2>/dev/null | tail -1 \
           | grep -oE '[0-9.]+' | tail -1)"
  elif pgrep -f "src.train.*nut_fastening_v21_stage2" >/dev/null 2>&1; then
    steps="$(grep -a 'total_timesteps' runs/nut_fastening_v21_stage2.log 2>/dev/null | tail -1 \
             | grep -oE '[0-9]+' | tail -1)"
    fps="$(grep -a '|    fps' runs/nut_fastening_v21_stage2.log 2>/dev/null | tail -1 \
           | grep -oE '[0-9.]+' | tail -1)"
    log "stage2 active  steps=${steps:-?}  fps=${fps:-?}"
  else
  # Neither stage running — check if we're done or need restart
    if [[ -f runs/nut_fastening_v21_stage2/final.zip ]]; then
      log "ALL DONE — stage2 final exists"
      break
    elif [[ -f runs/nut_fastening_v21_stage1/final.zip ]] \
         && ! pgrep -f "chain_v21_stage2_after_stage1" >/dev/null 2>&1; then
      log "stage1 done but stage2 not started — launching chain/stage2"
      setsid bash scripts/run_b_nut_train_v21_stage2.sh >> runs/nut_fastening_v21_stage2.log 2>&1 &
      disown
    else
  ...
  # --- error scan ---
  if grep -aqE "Traceback|MemoryError|Killed" runs/nut_fastening_v21_stage1.log 2>/dev/null; then
